...
class Detector:
    ...
    def detect_collisions(
        ...
        floor_data = self.extract_data(floor, cnt_min_area=1000)
        for d in floor_data:
            (x, y, w, h) = d['bbox']
            cx, cy = d['cx'], d['cy']
        ...
        box_data = self.extract_data(box, dilate_iter=5)
        for d in box_data:
            (x, y, w, h) = d['bbox']
            cx, cy = d['cx'], d['cy']
        ...
        collision = 0
        if self.check_for_collision(left):
            collision = -1
        elif self.check_for_collision(right):
            collision = 1
